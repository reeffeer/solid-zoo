...
- **Жёсткая привязка отчётного модуля к строковым константам вида** (пример: подсчёт львов по `species == "Lion"`). При добавлении сложной классификации (по группам, опасности, климату) может понадобиться выделить отдельные абстракции (например, `AnimalCategory`) и пересмотреть отчётный модуль.
